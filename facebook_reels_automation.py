"""
Facebook Reels Automation - Bilingual English/Italian Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Italian branding
"""
...
# Italian translations for display
CATEGORIES_ITALIAN = {
    ...
    "Wisdom": "Saggezza",
    "Happiness": "Felicità",
    "Self Improvement": "Miglioramento Personale",
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    category_italian = CATEGORIES_ITALIAN[category_english]
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Italian.
...
2. Italian translation (with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are an Italian teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "italian": "Credi in te stesso.", "pronunciation": "KREH-dee in teh STEH-sso."},
            {"english": "You are capable of amazing things.", "italian": "Sei capace di cose straordinarie.", "pronunciation": "SAY kah-PAH-cheh dee KOH-zeh stra-or-dee-NAH-ree-eh."},
            {"english": "Dream big, start small.", "italian": "Sogna in grande, inizia in piccolo.", "pronunciation": "SOH-nyah in GRAHN-deh, ee-NEE-tseeah in PEEK-koh-loh."},
            {"english": "Your future is created by your actions.", "italian": "Il tuo futuro è creato dalle tue azioni.", "pronunciation": "EEL TOO-oh foo-TOO-roh eh kreh-AH-toh DAHL-leh too-eh ah-TSYOH-nee."},
            {"english": "Never give up on your dreams.", "italian": "Non arrenderti mai ai tuoi sogni.", "pronunciation": "Nohn ah-RREN-dehr-teh my ah-ee TOO-oy SOH-nyee."},
        ],
        "Love": [
            {"english": "Love yourself first.", "italian": "Ama te stesso per primo.", "pronunciation": "AH-mah teh STEH-sso pehr PREE-moh."},
            {"english": "Love makes everything possible.", "italian": "L'amore rende tutto possibile.", "pronunciation": "Lah-MOH-reh REHN-deh TOOT-toh pohs-SEE-bee-leh."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        it_success = asyncio.run(generate_single_audio(phrase["italian"], ITALIAN_VOICE, str(italian_file)))
        if it_success:
            print(f"    ✓ Italian: {italian_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(italian_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        it_duration = get_audio_duration(str(italian_file))

        # Add pause between English and Italian
        pause_between = 0.5
        total_duration = en_duration + pause_between + it_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + IT: {it_duration:.2f}s)")
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{italian_file.as_posix()}'\n")
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    italian = phrase_data.get("italian", "")
    ...
    # Italian text
    italian_y = english_y + total_height + 110  # Increased from 100
    italian_lines = wrap_text(italian, font_large, VIDEO_WIDTH - 140)
    total_height = len(italian_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, italian_y - 55), (VIDEO_WIDTH - 60, italian_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(italian_lines):
        y_pos = italian_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = italian_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_ITALIAN[category_english]})")
    print(f"{'='*80}\n")
    ...
    for i, phrase in enumerate(phrases, 1):
        print(f"  {i}. {phrase['english']} → {phrase['italian']}")
    ...
    print("\n[3/4] Generating audio (English + Italian with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Italian")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇮🇹 VELOCITY ITALIAN - FACEBOOK REELS AUTOMATION 🇮🇹")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Italian branding at bottom")
    ...
    for i, cat in enumerate(CATEGORIES_ENGLISH, 1):
        print(f"   {i:2d}. {cat} ({CATEGORIES_ITALIAN[cat]})")
    print(f"\n📅 DAILY CAPACITY:")
